...
/// <summary>
/// Seeds initial data ONLY if it doesn't already exist in the database.
/// This is idempotent — running it multiple times will never overwrite
/// changes made from within the Admin Dashboard or the apps.
/// </summary>
public static class DbInitializer
{
    public static async Task Seed(ApplicationDbContext context)
    {
        ...
        // ─── 2. SEED TEST DRIVER (only if missing) ──────────────────────────
        var driverEmail = "tlhoweatisang+driver@gmail.com";
        if (!await context.Users.AnyAsync(u => u.Email == driverEmail))
        {
            var driver = new User
            {
            ...
            if (!await context.DriverDetails.AnyAsync(d => d.UserId == driver.Id))
            {
        ...
        // ─── 3. SEED TEST CUSTOMER (only if missing) ────────────────────────
        ...
        // ─── 4. SEED MERCHANT: Kitchen Hooligans (only if missing) ──────────
        // NOTE: Location, commission, logo and all fields are ONLY set on first
        // creation. All admin edits made inside the dashboard are preserved.
        if (!await context.Merchants.AnyAsync(m => m.Name == "Kitchen Hooligans"))
        {
            ...
            // Only seed menu items if the merchant was just created
        ...
        // ─── 5. SEED MERCHANT: K & D Delight (only if missing) ─────────────
